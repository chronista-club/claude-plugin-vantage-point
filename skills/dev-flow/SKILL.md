...
| `mcp__vantage-point__lane_nudge` / `vp lane nudge` | nudge | performer / conductor に text + Enter 注入 (旧 `tmux send-keys`) |
...
conductor の orchestration = 「どの performer から control を手放し、 どこで握り直すか」 の判断連鎖。 5 state FSM で wire pattern から derive、 conductor は performer 群の control 状態を一望し、 必要な performer にだけ介入する。 これは **「複数 performer を同時並走させながら conductor の認知 cost を一定に保つ」** 構造で、 並列 dev の規模拡張を可能化。
...
- 5 state FSM の transitional 検出 (= stuck → conductor 介入 trigger、 hitl_pending の timeout 等)
